...
function execute_sql_in_oracle ($sql) {
  ...
  $connection = oci_connect ("gq034", "adrohs", "gqiannew2:1521/pdborcl");
  if ($connection == false){
    // failed to connect
...
function display_oracle_error_message ($resource) {
  ...
  else if ($err['code'] == 20000)
    echo ("All seats are filled.  You can not enroll at this time. <BR /><BR />");
...
function get_GPA ($userid) {
  ...
  oci_bind_by_name($cursor, ":userid", $userid, 5);
  oci_bind_by_name($cursor, ":average", $gpa, 5);
