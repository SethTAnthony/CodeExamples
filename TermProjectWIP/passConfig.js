...
function config(app) {
    ...
    //user = {email, fullname, password: {sald, hashed}}
    const localLogin = new LocalStrategy(
        ...
        (req, email, password, done) => {
            ...
            app.locals.customersCollection.find({ email }).toArray()
                .then(users => {
                    if (users.length != 1) {
            ...
            app.locals.customersCollection.find({ email }).toArray()
                .then(users => {
                    if (users.length != 0) {
                        ...
                        app.locals.customersCollection.insertOne(user)
                            .then(result => {
...
function userSerialDeserial(app) {
    ...
    passport.deserializeUser((serial_user, done) => {
        app.locals.customersCollection.find({ _id: app.locals.ObjectID(serial_user) }).toArray()
            .then(users => {
                if (users.length != 1) {
